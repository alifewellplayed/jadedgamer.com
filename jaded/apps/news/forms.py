...
class NewsItemInstanceModelForm(forms.ModelForm):
    title = forms.CharField(
        max_length=250, widget=forms.TextInput(attrs={"class": "required form-control", "placeholder": "Post Title",}),
    )

    note = forms.CharField(
        max_length=250,
        required=False,
        widget=forms.Textarea(attrs={"class": "form-control", "placeholder": "Optional Note",}),
    )
